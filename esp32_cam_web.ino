...
void motorA(int dir, int spd) {
    ...
    ledcWrite(LEDC_CH_ENA, dir == 0 ? 0 : trimmed);
...
void motorB(int dir, int spd) {
    ...
    ledcWrite(LEDC_CH_ENB, dir == 0 ? 0 : trimmed);
    // CORE 3.x:  ledcWrite(ENB, dir == 0 ? 0 : trimmed);
...
void servoWrite(int angle) {
    ledcWrite(LEDC_CH_SERVO, angleToDuty(angle));
    // CORE 3.x:  ledcWrite(SERVO_PIN, angleToDuty(angle));
}
...
void setup() {
    ...
    // ---- LEDC PWM setup (Core 2.x API) ----
    //
    //  Core 2.x:
    //    ledcSetup(channel, freq, resolution);
    //    ledcAttachPin(pin, channel);
    //
    //  Core 3.x (replace the 4 lines below with):
    //    ledcAttach(ENA, PWM_FREQ, PWM_RESOLUTION);
    //    ledcAttach(ENB, PWM_FREQ, PWM_RESOLUTION);
    //
    ledcSetup(LEDC_CH_ENA, PWM_FREQ, PWM_RESOLUTION);
    ledcAttachPin(ENA, LEDC_CH_ENA);

    ledcSetup(LEDC_CH_ENB, PWM_FREQ, PWM_RESOLUTION);
    ledcAttachPin(ENB, LEDC_CH_ENB);

    // ---- Servo via LEDC (no library — avoids channel conflicts) ----
    //  Core 3.x:  ledcAttach(SERVO_PIN, SERVO_FREQ, SERVO_RES);
    ledcSetup(LEDC_CH_SERVO, SERVO_FREQ, SERVO_RES);
